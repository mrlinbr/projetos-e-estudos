...
totalsubvinte=0
for i in range (1,5):
    n = str(input('Qual o nome da {} pessoa? '.format(i))).strip()
    ida = int(input('Qual a idade da {} pessoa?'.format(i)))
    idade += ida
    sexo = str(input('Qual o sexo com qual a {} pessoa se identifica? [M/F]'.format(i))).strip()
    if  i==1 and sexo in 'Mm':
        idademaisvelho=idade
        nomemaisvelho=nome

    if sexo in 'Mm' and idade>idademaisvelho:
        idademaisvelho=idade, nomemaisvelho=nome

    if sexo in 'Ff' and idade<= 20:
        femsubvinte=idade,femsubvintenome=nome

    if sexo in 'Ff' and idade<= femsubvinte:
        femsubvinte=idade,femsubvintenome=nome
    if idade<20:
        totalsubvinte=idade

print('A media de idade do grupo é´{}'.format(idade/4))
print(' o homem mai velh do grupo é {}e tem a idade de {}'.format(nomemaisvelho,idademaisvelho))
print('A mulher mais nova do grupo é {} e se chama {}'.format(femsubvinte,femsubvintenome))
print('{} mulheres do frupo tem menos que 20 anos'.format(totalsubvinte))
'''
